...
class GraphqlDocs extends Component {
  render() {
    return (
      <div className="outer__container">
        ...
        <main className="inner__container">
          ...
          <p>
            <span className="bold">Note:</span> Every GraphQL service defines a
            set of types which completely describe the set of possible data you
            can query on that service. Then, when queries come in, they are
            validated and executed against that schema.
          </p>
          <h4>b. Arguments</h4>
          ...
          <p>
            All arguments in GraphQL are passed a specific name that is defined
            at the schema level. In this case, the <code>id</code> field has one
            defined argument, <code>ID!</code>.
            <br />
            <code>!</code> means that the field is non-nullable, meaning that
            the GraphQL service promises to always give you a value when you
            query this field. In the type language, we'll represent those with
            an exclamation mark.
          </p>
          <p>
          ...
          <p>
            ...
            Ok that's cool but...<em>why do we need them?</em> With aliases, we
            can query hero multiple times without fear of it being overridden in
            the result object. Without it, we wouldn't be able to query hero
            multiple times without it being overridden.
          </p>
          <img src={img__alias} alt="" />
          <p>
            In the above example, the two hero fields would have conflicted, but
            since we can alias them to different names, we can get both results
            in one request.
          </p>
          <h4>
    ...
    friends {
      name
          ...
          <p>
            ...
            side to identify different GraphQL requests.
          </p>
          <h4>f. Variables</h4>
          <p>
            ...
            dynamic. We can simply pass a different variable rather than needing
            to construct an entirely new query. This is also in general a good
            practice for denoting which arguments in our query are expected to
            be dynamic - we should never be doing string interpolation to
...
`}
          ...
          <p>
            ...
            <code>($episode: Episode)</code> in the query above. It works just
            like the argument definitions for a function in a typed language. It
            lists all of the variables, prefixed by <code>$</code>, followed by
            their type, in this case <code>Episode</code>.
          </p>
          <p>
            All declared variables must be either <b>scalars</b>, <b>enums</b>,
            or <b>input</b> object types. So if you want to pass a complex
            object into a field, you need to know what input type that matches
            on the server.{' '}
            <i>
          ...
          <p>
            ...
            type, it's optional. But if the field you are passing the variable
            into requires a non-null argument, then the variable has to be
            required as well.
          </p>
          <p>
    ...
    friends {
      name
          ...
          <p>
            ...
            query without passing any variables. If any variables are passed as
            part of the variables dictionary, they will override the defaults.
          </p>
          <h4>g. Directives: an official or authoritative instruction.</h4>
          <p>
            A directive can be attached to a field or fragment inclusion, and
            can affect execution of the query in any way the server desires. The
            core GraphQL specification includes exactly two directives, which
            must be supported by any spec-compliant GraphQL server
            implementation:
          </p>
...
`}</pre>
          ...
          <div className="quote">
            <ul>
              <li>
                @include(if: Boolean) Only include this field in the result if
          ...
          <p>
            Directives can be useful to get out of situations where you
            otherwise would need to do string manipulation to add and remove
